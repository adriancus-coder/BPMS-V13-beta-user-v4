...
const voiceLocales = {
  ro: 'ro-RO', no: 'nb-NO', ru: 'ru-RU', uk: 'uk-UA', en: 'en-US', es: 'es-ES',
  fr: 'fr-FR', de: 'de-DE', it: 'it-IT', pt: 'pt-PT', pl: 'pl-PL', tr: 'tr-TR', ar: 'ar-SA', fa: 'fa-IR', hu: 'hu-HU'
};
...
function setStatus(text) { $('participantStatus').textContent = text; }
function setParticipantUpdating(show) { $('participantUpdatingBadge').style.display = show ? 'block' : 'none'; }
...
function getTextForEntry(entry) { return entry?.translations?.[state.currentLanguage] || entry?.original || ''; }
function getHistoryEntries() {
  const entries = sortEntries(state.currentEvent?.transcripts || []);
  if (entries.length <= 1) return [];
  const result = []; let totalChars = 0;
  for (let i = entries.length - 2; i >= 0; i--) {
    const entry = entries[i]; const text = String(getTextForEntry(entry) || '').trim(); if (!text) continue;
    ...
    result.push(entry); totalChars = nextChars; if (result.length >= HISTORY_MAX_ITEMS) break;
  }
  return result;
}
function detectPreferredSupportedLanguage(available = []) {
  ...
  for (const raw of candidates) {
    ...
    for (const short of available) if (code.startsWith(short)) return short;
  }
...
function syncLanguageOptions(event) {
  ...
  available.forEach((code) => {
    ...
    option.value = code; option.textContent = langLabel(code); select.appendChild(option);
  });
  if (!state.languageInitialized) { select.value = detectPreferredSupportedLanguage(available); state.languageInitialized = true; }
...
function speakLatestEntry(entry) {
  ...
  const text = String(getTextForEntry(entry) || '').trim(); if (!text) return;
  stopSpeech();
  try {
    const utter = new SpeechSynthesisUtterance(text);
    const { locale, voice } = getVoiceForCurrentLanguage(); utter.lang = locale; if (voice) utter.voice = voice;
...
function renderLiveView({ announce = false } = {}) {
  if (!state.currentEvent) return;
  const latestEntry = getLatestEntry(); state.lastLiveEntryId = latestEntry?.id || null;
  $('lastText').textContent = latestEntry ? getTextForEntry(latestEntry) : 'Waiting for translation...';
  renderHistory(); updateTopMeta();
  if (announce && latestEntry && latestEntry.id !== state.lastSpokenEntryId) { state.lastSpokenEntryId = latestEntry.id; speakLatestEntry(latestEntry); }
}
function updateEntryInState(payload) {
  const entry = getEntryById(payload.entryId); if (!entry) return;
  entry.sourceLang = payload.sourceLang; entry.original = payload.original; entry.translations = payload.translations || {}; entry.edited = true;
}
...
async function joinParticipantEvent() {
  ...
  socket.emit('join_event', { eventId, role: 'participant', language: $('languageSelect')?.value || state.currentLanguage, participantId: state.participantId });
}

socket.on('connect', async () => { setStatus('Connecting...'); await joinParticipantEvent(); });
socket.on('disconnect', () => setStatus('Reconnecting...'));
socket.on('join_error', ({ message }) => setStatus(message || 'Cannot join event.'));
socket.on('joined_event', ({ event, role }) => {
  if (role !== 'participant') return;
  state.currentEvent = event; state.serverAudioMuted = !!event.audioMuted;
  syncLanguageOptions(event); renderLiveView({ announce: false }); setParticipantUpdating(false);
...
socket.on('transcript_entry', (entry) => {
  ...
  setParticipantUpdating(false); renderLiveView({ announce: true });
});
socket.on('transcript_source_updated', (payload) => { updateEntryInState(payload); setParticipantUpdating(false); renderLiveView({ announce: false }); });
...
socket.on('audio_state', ({ audioMuted }) => { state.serverAudioMuted = !!audioMuted; if (audioMuted) { stopSpeech(); setStatus('Audio stopped by admin.'); } else { setStatus(state.localAudioEnabled ? 'Audio active.' : 'Local audio paused.'); } });
socket.on('active_event_changed', async () => { if (!state.fixedEventId) await joinParticipantEvent(); });
socket.on('mode_changed', ({ mode }) => { if (mode === 'song') setStatus('Song mode active on public screen.'); });

$('languageSelect').addEventListener('change', handleLanguageChange);
$('playAudioBtn').addEventListener('click', () => { state.localAudioEnabled = true; setStatus(state.serverAudioMuted ? 'Audio stopped by admin.' : 'Local audio active.'); const latestEntry = getLatestEntry(); if (latestEntry) speakLatestEntry(latestEntry); });
$('pauseAudioBtn').addEventListener('click', () => { state.localAudioEnabled = false; stopSpeech(); setStatus('Local audio paused.'); });

window.addEventListener('load', async () => {
  try { const res = await fetch('/api/languages'); const data = await res.json(); availableLanguages = data.languages || {}; } catch (_) {}
