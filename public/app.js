...
async function syncSpeedToEvent() {
  ...
  const res = await fetch(`/api/events/${currentEvent.id}/settings`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ speed }) });
  const data = await res.json();
...
function renderSongLibrary(items = []) {
  ...
  box.innerHTML = items.map((item) => `
    <div class="event-card">
      ...
      <div class="meta">${escapeHtml((item.text || '').slice(0, 140))}${(item.text || '').length > 140 ? '...' : ''}</div>
      <div class="actions">
  ...
  box.innerHTML = items.map((item) => {
    ...
    return `
      <div class="history-item">
        ...
        <div class="small">${escapeHtml(preview.slice(0, 160))}${preview.length > 160 ? '...' : ''}</div>
      </div>
    `;
  }).join('');
}
...
async function sendSongToLive() {
  ...
  const res = await fetch(`/api/events/${currentEvent.id}/display/manual`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, text })
  });
  const data = await res.json();
...
async function setDisplayMode(mode) {
  if (!currentEvent) return;
  const res = await fetch(`/api/events/${currentEvent.id}/display/mode`, {
...
async function setEventMode(mode) {
  if (!currentEvent) return;
  const res = await fetch(`/api/events/${currentEvent.id}/mode`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode }) });
  const data = await res.json();
  if (data.ok) {
    currentEvent = data.event;
    renderActiveEventBadge(currentEvent);
...
$('saveGlossaryBtn').addEventListener('click', async () => {
  ...
  const res = await fetch(`/api/events/${currentEvent.id}/glossary`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source, target, lang, permanent }) });
  const data = await res.json();
...
$('saveSourceCorrectionBtn').addEventListener('click', async () => {
  ...
  const res = await fetch(`/api/events/${currentEvent.id}/source-corrections`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ heard, correct, permanent }) });
  const data = await res.json();
...
$('songLibraryList').addEventListener('click', async (e) => {
  ...
  if (action === 'delete') {
    ...
    const res = await fetch(`/api/events/${currentEvent.id}/song-library/${songId}`, { method: 'DELETE' });
    const data = await res.json();
    if (!data.ok) return;
    currentEvent.songLibrary = data.songLibrary || [];
    renderSongLibrary(currentEvent.songLibrary);
...
$('openSongScreenBtn').addEventListener('click', () => { const url = $('songLink').value || '/translate'; if (url) window.open(url, '_blank'); });
