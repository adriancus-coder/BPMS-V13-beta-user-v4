...
async function createEvent({ name, speed, sourceLang, targetLangs, baseUrl, scheduledAt }) {
  ...
  const songLink = `${baseUrl}/translate?event=${id}`;
...
async function buildTranslationsForAllTargets(text, event) {
  ...
  const translationPairs = await Promise.all(
    (event.targetLangs || []).map(async (lang) => [lang, await translateText(clean, lang, event)])
  );
...
function pushSongHistory(event, item) {
  ...
  event.songHistory.unshift({
    ...
    source: sanitizeTranscriptText(item.source || ''),
...
app.post('/api/events/:id/display/manual', async (req, res) => {
  ...
  const text = sanitizeTranscriptText(req.body.text || '');
  const title = String(req.body.title || '').trim();
...
app.post('/api/events/:id/song-library', (req, res) => {
  ...
  const existingIndex = event.songLibrary.findIndex((item) => item.title.toLowerCase() === title.toLowerCase());

  const payload = {
    id: existingIndex >= 0 ? event.songLibrary[existingIndex].id : randomUUID(),
    title,
    text,
    updatedAt: new Date().toISOString()
  };
  ...
  if (existingIndex >= 0) {
    event.songLibrary[existingIndex] = payload;
  } else {
    event.songLibrary.unshift(payload);
  }

  if (event.songLibrary.length > 100) {
    event.songLibrary = event.songLibrary.slice(0, 100);
  }
